...
function calcular() {
    ...
    for(let item of elementosVentas.children) {
        let campoInput = item.children[1]
        let valorVenta = extraerNumeroDesdeElemento(campoInput)
        campoInput.setAttribute('style', 'background-color: white')
        if (valorVenta == ventaMayor){
            campoInput.setAttribute('style', 'background-color: lightgreen')
        } else {
            if (valorVenta == ventaMenor){
                campoInput.setAttribute('style', 'background-color: red')
            }
        }
    }
